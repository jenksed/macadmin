...
# Wire in logging + exit codes (safe to re-source)
{
  local _dir=${(%):-%N}
  _dir=${_dir:A:h}
  [[ -r "$_dir/log.zsh" ]] && source "$_dir/log.zsh"
} || true
{
  local _dir=${(%):-%N}
  _dir=${_dir:A:h}
  [[ -r "$_dir/exitcodes.zsh" ]] && source "$_dir/exitcodes.zsh"
} || true
...
warn()
{
  ...
  print -r -- "${C[yellow]}[WARN]${C[reset]} $*" >&2
}
error()
{
  ((${MACADMIN_JSON:-0})) && {
